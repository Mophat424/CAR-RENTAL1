...
// import * as insuranceService from "../../src/insurance/insuranceService";
...
describe("Insurance API Integration Tests", () => {
  ...
  it("should return 500 when creating with missing fields", async () => {
    const res = await request(app).post("/insurance").send({});
    expect(res.statusCode).toBe(500);
  });

  it("should catch error thrown during creation", async () => {
    const spy = jest
      .spyOn(insuranceService, "createInsuranceService")
      .mockImplementationOnce(() => {
        throw new Error("Forced error");
  ...
  it("should return 404 when no insurance records exist", async () => {
    await db.delete(InsuranceTable);
    const res = await request(app).get("/insurance");
    expect(res.statusCode).toBe(404);
  });

  it("should fetch insurance by ID", async () => {
    ...
    insuranceID = inserted.insuranceID;
  ...
  it("should return 400 for invalid insurance ID format", async () => {
    const res = await request(app).get("/insurance/notanid");
    expect(res.statusCode).toBe(400);
  });
  ...
  it("should return 404 for invalid delete ID format", async () => {
    const res = await request(app).delete("/insurance/invalid-id");
    expect(res.statusCode).toBe(404);
  });

  it("should catch error thrown during deletion", async () => {
    const [inserted] = await db
      .insert(InsuranceTable)
      .values({
        carID: testCarID,
    ...
    const res = await request(app).delete(`/insurance/${inserted.insuranceID}`);
    expect(res.statusCode).toBe(500);
