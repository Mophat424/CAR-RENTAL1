...
describe("Maintenance Routes Integration Tests", () => {
  ...
  describe("PUT /maintenance/:id", () => {
    it("should update a maintenance record", async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.message).toBe("Maintenance record updated successfully");
    });
  ...
  describe("DELETE /maintenance/:id", () => {
    it("should delete the maintenance record", async () => {
      const newMaintenance = await db.insert(MaintenanceTable).values({
        carID: carId,
        maintenanceDate: "2025-08-01",
        description: "To delete",
        cost: "40.00",
      }).returning();
      const newId = newMaintenance[0].maintenanceID;

      const res = await request(app).delete(`/maintenance/${newId}`);
      expect(res.status).toBe(200);
